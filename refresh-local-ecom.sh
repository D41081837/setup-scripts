...
echo -e "\n"
echo -e "${BLUE}ATGE ECOM Local Development Refresh v1.2${NC}"
...
select refresh in "${options[@]}"; do
    ...
    echo $REFRESHTYPE >> ~/setup-scripts/.setup_vars;
  case "$reftype,$REPLY" in
    SYNC,*|*,SYNC)     bash ~/setup-scripts/refresh-sync-vm.sh; break ;;
    REBUILD,*|*,REBUILD)     bash ~/setup-scripts/refresh-reload-vm.sh; break ;;
  esac
done
